...
char pic_read_touch(void){
	return iic_data.Touch_Data;
}

unsigned short pic_read_touch_dataL(void){
//	return iic_data.TCNT.WORD;
	return iic_data.TCNT.BYTE.TCNT_L;
}

unsigned short pic_read_touch_dataH(void){
//	return iic_data.TCNT.WORD;
	return iic_data.TCNT.BYTE.TCNT_H;
}

unsigned short pic_read_touch_th_H(void){
//	return iic_data.TCNT.WORD;
	return iic_data.TCNT_TH.BYTE.TCNT_TH_H;
}

unsigned short pic_read_touch_th_L(void){
//	return iic_data.TCNT.WORD;
	return iic_data.TCNT_TH.BYTE.TCNT_TH_L;
}
...
void init_I2C(void)
{
	...
	RIIC2.ICMR1.BYTE = 0x38;//bitcounter=9 PCLK/4=10MHz 0.08us 		28
	RIIC2.ICBRL.BYTE = 0xFD;//転送速度=1/{[ICBRH+1)+(ICBRL+1)]/IICφ}	F0
	RIIC2.ICBRH.BYTE = 0xF8;//IICφ=PCLKx10^6×分周比　Tr=0の場合は400kHz	E7
...
void ENC_WHO(void){
	unsigned char who;
	I2C_START();
	I2C_PUT((0x36)<<1);//アドレス write
	I2C_PUT(0x09);//レジスタ
	I2C_RESTART();
	I2C_PUT((0x36<<1)|0x01);//アドレス read
...
void ENC_SETTING(void){
//ポーリングによる設定
//*
	I2C_START();//Startbit発生
	I2C_PUT((0x36<<1));//アドレス write
	I2C_PUT(0x09);//PWR_MGMT_2
	I2C_PUT(0x07);//sleep解除
	I2C_STOP();
//*/

/*
	I2C_START();//Startbit発生
	I2C_PUT((0x68<<1));//アドレス write
	I2C_PUT(0x6c);//PWR_MGMT_2
	I2C_PUT(0x0);//sleep解除
	I2C_STOP();

	I2C_START();//Startbit発生
	I2C_PUT((0x68<<1));//アドレス write
	I2C_PUT(0x1B);//scale range 
	I2C_PUT(0x18);//±2000dps
	I2C_STOP();
//*/
}
...
/*
	I2C_START();//Startbit発生
	I2C_PUT((0x08<<1));//アドレス write
	I2C_PUT(0x06);//PWR_MGMT_2
	I2C_PUT(0x01);//sleep解除
	I2C_STOP();
*/
...
//割り込み許可
	...
	RIIC2.ICIER.BYTE = 0xBB;//TIE RIE NAKIE SPIE ALIE TMOIE enable
...
void GYRO_Z_INT(void){
	if(RIIC0.ICCR2.BIT.BBSY !=0){
		return;
	}
	iic_buff.SlvAdr=0x68;
	iic_buff.RW=1;//READ
	iic_buff.RA=0x47;
	iic_buff.RWCnt=2;//データ数
//	iic_buff.pRWData= &iic_data.GYRO_Z;
	...
	RIIC0.ICCR2.BIT.ST = 1;//割り込みフラグ:START 割り込み要因:EEI
}

void ACCE_Y_INT(void){
	if(RIIC0.ICCR2.BIT.BBSY !=0){
		return;
	}
//	GYRO_ACCE_F=1;
//	iic_buff.RWCnt=2;
//	iic_buff.pRWData= &I2C_BUF[0];
//	iic_trm_cnt=0;/* Clear the internal transmission counter for IIC */
//	iic_rcv_cnt=0;/* Clear the internal reception counter for IIC */
//	RIIC2.ICSR2.BIT.START = 0;
//	RIIC0.ICCR2.BIT.ST = 1;//割り込みフラグ:START 割り込み要因:ICEEI
}
...
void GYRO_Z(void){
	...
	I2C_START();
	I2C_PUT(0xd6);//アドレス write
	I2C_PUT(0xac);//レジスタ out_z_L 0x2c
	I2C_RESTART();
	I2C_PUT(0xd7);//アドレス read

	while(RIIC0.ICSR2.BIT.RDRF==0);
	if(RIIC0.ICSR2.BIT.NACKF==1){	RIIC0.ICSR2.BIT.STOP=0;}

	tmp=RIIC0.ICDRR;//dummy
