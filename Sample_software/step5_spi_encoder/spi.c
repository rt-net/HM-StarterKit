...
void init_spi_gyro(void)
{
	...
	*****************************************************************************************/
	...
	RSPI1.SPDCR.BIT.SPLW =	 0x00;	//SPDRレジスタへはワードアクセス(SPLE = 1の時はロングワードアクセス)
	...
	//割り込みの禁止(優先レベル設定のために一度禁止)
	ICU.IER[5].BIT.IEN2 = 0;	//SPRI1の割り込み禁止
	ICU.IER[5].BIT.IEN3 = 0;	//SPTI1の割り込み禁止
	ICU.IER[5].BIT.IEN4 = 0;	//SPII1の割り込み禁止
	...
	PORTE.PMR.BIT.B4 = 	0x00;	//PE4をIOとして使用
	PORTE.PDR.BIT.B4 = 	0;	//PE4を入力ポートとして使用
	...
	//RSPI制御レジスタの設定
	RSPI1.SPCR.BIT.SPMS = 		0x01;	//クロック同期式動作
...
void read_spdr_gyro(void)
{
	...
	*****************************************************************************************/
	read++;
	gyro_data = RSPI1.SPDR.WORD.H;	//データの読み込み
	//gyro_data = 0xffff;
